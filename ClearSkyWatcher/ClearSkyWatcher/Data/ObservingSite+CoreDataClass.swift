...
public class ObservingSite: NSManagedObject {
    ...
    typealias ObservingSiteData = (key: String, name: String, latitude: Float?, longitude: Float?, regionName: String)
...
//    }
    ...
    class func updateOrCreateSites(withRawData observingSiteData: (sortedKeys: [String], sortedProps: [String]), callbackOnComplete: ((Bool) -> Void)?) {
        let parsingGroup = DispatchGroup()
        let dispatchQueue = DispatchQueue.global(qos: .userInitiated)
        var parsedData = [ObservingSiteData]()
        let arrayDispatchQueue = DispatchQueue(label: "ArrayDispatchQueue")
        func addToParsedData(data: ObservingSiteData) {
            arrayDispatchQueue.async(group: nil, qos: .userInitiated, flags: .barrier) {
        ...
        for index in 0..<observingSiteData.sortedKeys.count {
            dispatchQueue.async {
                ...
                if !currentKeyString.isEmpty && !currentPropString.isEmpty {
                    addToParsedData(data: ObservingSite.parse(keyString: currentKeyString, propString: currentPropString))
                }
                parsingGroup.leave()
            }
        }
        
        parsingGroup.notify(queue: DispatchQueue.global(qos: .userInitiated)) {
            PersistenceManager.shared.doAsync(block: { context in
                for observingSiteData in parsedData {
                ...
                if let callback = callbackOnComplete {
                    callback(true)
                }
            }
        }
    ...
    class func updateOrCreate(withData observingSiteData: ObservingSiteData, callbackOnComplete: (() -> Void)? = nil) {
        PersistenceManager.shared.doAsync(block: { context in
            let existingSite = PersistenceManager.shared.getObservingSite(withKey: observingSiteData.key)
    ...
    class func parse(keyString: String, propString: String) -> ObservingSiteData {
        var retTuple: ObservingSiteData
